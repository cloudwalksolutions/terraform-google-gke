...
module "gke" {
  ...
  database_encryption = local.use_kms_keys ? [
    {
      state    = "ENCRYPTED"
      key_name = module.kms[0].keys["primary"]
    }
  ] : []
  ...
  node_pools_taints       = var.node_pool_taints
...
module "failover_gke" {
  ...
  database_encryption = local.use_kms_keys ? [
    {
      state    = "ENCRYPTED"
      key_name = module.failover_kms[0].keys["failover"]
    }
  ] : []
